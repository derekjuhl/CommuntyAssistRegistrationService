...
public class LoginClass
{
    ...
    private void GetUserInfo()
    {
        ...
        //query the fields
        var info = from c in communityAssistDb.People
                   where c.PersonUsername.Equals(username)
                   select new
                   { c.Personpasskey, c.PersonUserPassword, c.PersonKey };

        //loop through the results and assign the
        //values to the field variables
        foreach (var u in info)
        {
            seed = (int)u.Personpasskey;
            dbhash = u.PersonUserPassword;
            key = u.PersonKey;
        }
    }
